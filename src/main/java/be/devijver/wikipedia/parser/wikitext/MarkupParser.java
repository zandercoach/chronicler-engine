...
/**
 * <p>
 * Parser for the Wikipedia markup language that produces an AST tree.
 * 
 * <p>
 * This parser is a mess, unfortunately, but it's well tested and has a very
 * good performance. It is however largely experimental and can only handle the
 * basic subset of Wikipedia markup.
 * 
 * <p>
 * Currently this parser recognizes this markup:
 * 
 * <ul>
 * <li>Heading of limitless depth: <code>==Article heading==</code>,
 ...
 * <li>Unordered list items of limitless depth: <code>* Item 1</code>,
 * <code>** Sub item 1</code>, ...
 * <li>Ordered list items of limitless depth: <code># Item 1</code>,
 * <code>## Sub item 1</code>, ...
 * <li>Combined ordered and unordered list items: <code>*# Item 1</code>
 * <li>Indenting of limitless depth: <code>: One indent</code>,
 * <code>:: Two indents</code>
 * <li>Normal links: <code>[http://www.google.com]</code>,
 * <code>[http://www.google.com Google]</code>
 * <li>'Smart' links: <code>[[Java programming language]]</code>,
 * <code>[[Java programming language|Java]]</code>
 * </ul>
 * 
 * <p>
 * <b>Note:</b> paragraphs, list items and indentions are terminated by a
 * newline character or when the end of the parsed text is reached.
 * 
 * <p>
 * See http://en.wikipedia.org/wiki/Help:Editing for the entire Wikipedia
 * markup.
 * 
 * <p>
 * This parser only parses the markup without giving it any meaning. See
 * <code>AbstractASTParser</code> which is double-pass parser based on the
 * output of this parser.
 * 
 * <p>
 * The main reason for the limited markup support of this parser is that the
 * Wikipedia markup is very rich and the original parser is written in OCaml, a
 * powerful functional programming language. Mimicing the same behavior in an
 * object-oriented programming language like Java results in a mess as the
 * source code of this parser demonstrates. This makes that adding new features
 * to this parser becomes harder and harder as it grows.
 * 
...
public class MarkupParser extends ParserSupport {
  ...
  private Content parsePre() {
    return parseWithTagHandler("pre", new TagHandler() {
  ...
  private Content parseWithTagHandler(String tagName, TagHandler tagHandler) {
    if (!isNextBeyondEnd()) {
      ...
      TagHandlerParserCallback callback = new TagHandlerParserCallback() {
  ...
  private Content parseNowiki() {
    return parseWithTagHandler("nowiki", new TagHandler() {
