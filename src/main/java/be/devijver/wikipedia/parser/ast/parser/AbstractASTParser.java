...
/**
 * Created by IntelliJ IDEA. User: steven Date: 5-nov-2006 Time: 16:39:15 To
...
public abstract class AbstractASTParser {
  ...
  public void parse(UnorderedListItem content, Visitor visitor, ParseContext context) {
    ...
    while (next != null && next instanceof SingleContentHolder) {
      ...
      if (nextContentHolder.getContent() instanceof UnorderedListItem
          || nextContentHolder.getContent() instanceof OrderedListItem) {
  ...
  public void parse(SpecialConstruct content, Visitor visitor, ParseContext context) {
    if ('|' == content.getCharacter()) {
      Table table = null;
      try {
        table = new JavaCCTableParser(new StringReader("{|" + content.getContent() + "|}")).parseTable();
  ...
  public void parse(Table table, Visitor visitor, ParseContext context) {
    ...
    if (table.getCaption() != null || table.getCaptionOptions() != null) {
      ...
      parse(parseMarkup(new Characters(table.getCaption())), visitor);
      visitor.endCaption();
    }
  ...
  public void parse(Cell cell, Visitor visitor, ParseContext context) {
    ...
    for (int i = 0; i < content.length; i++) {
      if (content[i] instanceof Characters) {
        parse(parseMarkup((Characters) content[i]), visitor);
      } else {
